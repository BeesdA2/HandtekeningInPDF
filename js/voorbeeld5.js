...
async function testHandtekening (origineelDocument, handTekening, xxText, yyText) {
   try{
	   
	   
	  
  const pdfDoc = await PDFDocument.load(fs.readFileSync(origineelDocument));
  
const img = await pdfDoc.embedPng(fs.readFileSync(handTekening));
// Embed the Times Roman font
const timesRomanFont = await pdfDoc.embedFont(StandardFonts.TimesRoman);
const origineelDocumentFilename = path.parse(origineelDocument).name;
 console.log('origineelDocumentFilename: '+ origineelDocumentFilename); 
...
const pdfBytes = await pdfDoc.save();
 var pathToPDF = "";
  const newFilePath = origineelDocumentFilename + '-handTekening.PDF'; 
...
testHandtekening (origineelDocument,handTekening, xxText, yyText);